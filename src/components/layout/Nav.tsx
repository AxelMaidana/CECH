...
export default function Component() {
      ...
      <li key={item.id} className={`w-full ${depth === 0 ? 'border-b border-black border-opacity-20' : ''}`}>
        <div
          className={`flex items-center justify-between px-2 py-3 transition-all duration-300 ease-in-out hover:shadow-lg
            ...
            cursor-pointer`}
          onClick={() => (children.length > 0 ? toggleMobileSubmenu(item.id) : toggleMenu())}
        >
  ...
  if (isLoading) {
    return (
      <nav className="bg-customGreen animate-pulse text-sm font-bold flex self-end lg:self-auto lg:justify-center lg:items-center rounded-l-xl lg:rounded-none lg:rounded-b-xl shadow-lg w-8 lg:w-full z-auto relative">
