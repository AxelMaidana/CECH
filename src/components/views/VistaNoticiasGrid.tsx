...
import VistaBotonCrearNoticia from '../views/VistaBotonCrearNoticia';

const Vista = () => {
  const [posts, setPosts] = useState([]); // Noticias a mostrar por página
  ...
  const resultsPerPage = 10; // Limite de noticias por página
  ...
  const fetchPosts = async (page = 1) => {
    ...
    let q = query(postsCollection, orderBy('date'), limit(resultsPerPage));

    // Si no es la primera página, usamos el último documento de la página anterior
    if (page > 1 && lastDocs[page - 1]) {
      q = query(postsCollection, orderBy('date'), startAfter(lastDocs[page - 1]), limit(resultsPerPage));
    }
  ...
  useEffect(() => {
    fetchTotalPosts();
    fetchPosts(); // Cargar las noticias iniciales (primera página)
  }, []);
...
const formatDate = (dateString: string) => {
  ...
  const options: Intl.DateTimeFormatOptions = { day: '2-digit', month: '2-digit', year: 'numeric' }; // Formato "dd/mm/yyyy"
      ...
      <div className="my-10">
        <div className='flex justify-between items-center'>
          <div className="relative flex-shrink-0">
            <h3 className="md:text-2xl font-bold bg-customGreen text-white md:px-8 md:py-2.5 px-2 py-2 rounded-r-full text-center">
              Noticias disponibles
            </h3>
          </div>
          {/* Campo de búsqueda */}
        <div className='flex gap-2 items-center md:flex-row flex-col md:gap-0'>
        <VistaBotonCrearNoticia />
          <div className="relative flex w-fit items-center md:ml-4 md:mr-5 border-2 border-customGreen rounded-3xl">
            <input
              type="text"
        ...
        <div className={`flex pt-12 mb-6 justify-center items-center w-full tracking-tight z-auto`}>
          <section className={`relative bg-customBlue text-white pl-4 md:px-32 py-4 -mt-5 w-full shadow-custom shadow-black/40`}>
            <h3 className="font-bold text-start md:text-start md:mb-2 text-2xl md:text-3xl">Acceso gratuito a las noticias</h3>
            <p className="mb-2 mt-2 text-left max-w-full">
            Bienvenido al panel de <strong>Noticias</strong> aqui podras encontrar <strong>todo</strong> sobre lo que paso en estos ultimos tiempos.
            </p>
            </section>
        </div>
        ...
        <div className="mt-6 flex justify-center items-center space-x-2">
          <button
            className={`px-4 py-2 bg-customBlue rounded-full ${currentPage === 1 ? 'cursor-not-allowed opacity-50 text-white' : 'text-white hover:bg-cyan-800'}`}
          ...
          <div className='flex gap-2 px-6'>
            {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
              <button
                key={page}
                className={`flex h-4 w-4 p-4 justify-center items-center rounded-full ${currentPage === page ? 'bg-customBlue text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
          ...
          <button
            className={`px-4 py-2 ml-6 bg-customBlue rounded-full ${currentPage === totalPages ? 'cursor-not-allowed opacity-50 text-white' : 'hover:bg-cyan-800 text-white'}`}
